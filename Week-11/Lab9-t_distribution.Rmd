...
2. Test the function using a sample of $10$ heights from the `yrbss` data. Make sure to remove NA's first:
...
3. Replicate the function $1000$ times and use `table()` to determine the percentage of confidence intervals that contain the population mean
...
6. Test the function using a sample of $10$ heights from the `yrbss` data.

```{r}
# insert code here
```

7. Replicate the function $1000$ times and use `table()` to determine the percentage of confidence intervals that contain the population mean.
...
10. In one line of code, take a sample of size $10$ from the heights data and find how many standard errors away from the population mean it is using the formula for $z$-scores: 
...
11. Replicate this $1000$ times to find $1000$ $z$-scores and save the results as a `data.frame()` named `z_scores` and a column named `observation`.

```{r}
# insert code here
```

12. Plot the $z$-scores in a histogram using a bin width of $.25$ and add a normal curve to the graph with mean $0$ and standard deviation $1$.
...
Now we will do the same steps, but istead of computing $z$-scores we will compute $t$-scores. The difference between the two is that the $t$-score uses the sample standard deviation.

13. In one line of code, take a sample of size $10$ from the heights data and find how many standard errors away from the population mean it is using the formula for $t$-scores: 
...
14. Replicate this $1000$ times to find $1000$ $t$-scores and save the results as a `data.frame()` named `t_scores` and a column named `observation`.

```{r}
# insert code here
```

15. Plot the $t$-scores in a histogram using a bin width of $.25$ and add a normal curve to the graph with mean $0$ and standard deviation $1$.
...
17. Take a sample of size $10$ from the heights data. Find the mean and standard deviation of your sample and save them as `sample_mean` and `sample_sd`. Just like in the central limit theorem, we find the standard error by dividing the standard deviation by $sqrt(n)$, save this as `SE`. 
...
The $9$ in the code is because we are taking samples of size $10$ and so we use the $t$-distribution defined by degrees of freedom $=10-1$. So, in the $t$-distribution for $9$ degrees of freedom $95 \%$ of the data is between $-2.26$ and $2.26$ standard errors from the center. 
...
20. Test the function using a sample of $10$ heights from the `yrbss` data.

```{r}
# insert code here
```

21. Replicate the function $1000$ times and use `table()` to determine the percentage of confidence intervals that contain the population mean.
